                    ...
                    if not new_user_password.islower() and not new_user_password.isupper(): # check a lower and upper 
                        if len(set(string.digits) & set(new_user_password)) >=3: # check if there are a numbers
                            if len(set('!@#$?*') & set(new_user_password)) >=1: # check if there is one symbol
                                ...
                                print("You have not a special symbol like ('!','@','#','$','?','*',)",new_user_password,"\n\n")
                        else:
                            print("You have not a numbers, enter at least 3 numbers",new_user_password,"\n\n")
                    else:
                        print("Pleas enter a lower and upper words",new_user_password,"\n\n")
                else:
                    print("You have not a letters",new_user_password,"\n\n")
            else:
                print("You`re pass it too short!",new_user_password,"\n\n")